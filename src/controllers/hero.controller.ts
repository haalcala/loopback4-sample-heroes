...
import { Hero, Species, Planet } from "../models";
...
export class HeroController {
	...
	async count(@param.query.object("where", getWhereSchemaFor(Hero)) where?: Where<Hero>): Promise<Count> {
	...
	async find(@param.query.object("filter", getFilterSchemaFor(Hero)) filter?: Filter<Hero>): Promise<Hero[]> {
	...
	async updateAll(@requestBody() hero: Hero, @param.query.object("where", getWhereSchemaFor(Hero)) where?: Where<Hero>): Promise<Count> {
	...
	async updateById(@param.path.number("id") id: number, @requestBody() hero: Hero): Promise<void> {
	...
	async replaceById(@param.path.number("id") id: number, @requestBody() hero: Hero): Promise<void> {
	...
	async getFriend(@param.path.number("id") heroId: typeof Hero.prototype.id): Promise<Hero> {
	...
	async getPlanet(@param.path.number("id") heroId: typeof Hero.prototype.id): Promise<Planet> {
	...
	async getSpecies(@param.path.number("id") heroId: typeof Hero.prototype.id): Promise<Species> {
